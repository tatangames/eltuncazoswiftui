...
struct EditarProductoView: View {
    ...
    let idFilaCarrito: Int
    
    @StateObject private var viewModel = InformacionProductoEditarViewModel()
    @StateObject private var viewModelGuardar = ActualizarProductoEditadoViewModel()
    @StateObject private var toastViewModel = ToastViewModel()
    
    @State private var producto: ModeloInformacionProductoEditarArray? = nil
    @State private var cantidad: Int = 1
    @State private var nota: String = ""
    @State private var errorNotaObligatoria: Bool = false
    @State private var datosInicializados: Bool = false
    ...
    var body: some View {
                        ...
                        VStack(spacing: 0) {
                            ...
                            VStack(spacing: 12) {
                                ...
                                HStack(spacing: 0) {
                                    ...
                                    Button(action: { if cantidad < 50 { cantidad += 1 } }) {
                                        ZStack {
                                            Circle()
                            ...
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                .overlay(
                                    RoundedRectangle(cornerRadius: 12)
                                        .stroke(errorNotaObligatoria ? Color.red : Color.gray.opacity(0.25), lineWidth: 1)
                                )
                            ...
                            VStack(spacing: 16) {
                                ...
                                Button(action: {
                                    if prod.utiliza_nota == 1 && nota.trimmingCharacters(in: .whitespaces).isEmpty {
        ...
        .onReceive(viewModelGuardar.$loadingSpinner) { loading in openLoadingSpinner = loading }
    ...
    func cargarProducto() {
        viewModel.informacionProductoEditarRX(clienteid: idUsuario, carritoid: idFilaCarrito) { result in
            switch result {
            case .success(let modelo):
                if modelo.success == 1, let prod = modelo.producto {
                    self.producto = prod
                    if !self.datosInicializados {
                        self.cantidad = max(prod.cantidad, 1)
                        self.nota = prod.nota_producto ?? ""
                        self.datosInicializados = true
                    }
                } else {
    ...
    func serverActualizar() {
        viewModelGuardar.actualizarProductoRX(
            clienteid: idUsuario,
            cantidad: cantidad,
            carritoid: idFilaCarrito,
